...
class convert_tf():
    ...
    def myhook(self):
        ...
        t = time.time()
        np.save('Recording_' + str(t) + '.npy',self.out_vec)
        print('Number of points : ', len(self.out_vec))

    def write_to_file(self):
        self.out_vec.append([self.thumbPosePub.position.x,self.thumbPosePub.position.y,self.thumbPosePub.position.z])
        x = [self.thumbPosePub.position.x,self.thumbPosePub.position.y,self.thumbPosePub.position.z]
        # with open("test.txt", "a") as myfile:
        #     myfile.write(str(x))
        #     myfile.write('\n')        
        with open(self.out_string, 'a+') as outfile:
            np.savetxt(outfile,x) 
        # np.savetxt(f_handle, x, newline=" ")
        # f_handle.close()
    ...
    def init_topics(self):
        ...
        self.RobotCommandPub = rospy.Publisher(
            "/Robot/command_buffer", Float64MultiArray, queue_size=3)
    ...
    def init_params(self):
        ...
        self.grab=0
        t = time.time()
        self.out_string = 'Recording_'+str(t)+'.txt'
