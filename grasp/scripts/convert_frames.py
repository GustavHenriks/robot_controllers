...
class convert_frames():
    def __init__(self):

        freq = 100
        rospy.init_node('convert_frames', anonymous=True)
        ...
        print('Freezing the robot frame')
        delay = 1
        ...
        print("Converting frames at a frequency of " +str(freq) + " Hz")
        while not rospy.is_shutdown():
            # Setting palm link at the end of robot

            # world frame is the top tf published by iiwa controller
            self.br_robot_base_fixed.sendTransform(trans_world_base[0], q_world_base, rospy.Time.now(),
                                                'world', "mocap_world")
            
            # Used for testing target pos from pointcloud
            # self.br_robot_base_fixed.sendTransform(np.array([-1.2,-0.35,0.85]), q_world_base, rospy.Time.now(),
            #                         'target_position', "mocap_world")
            ...
            # self.desired_velocity = self.current_position - self.target_position
